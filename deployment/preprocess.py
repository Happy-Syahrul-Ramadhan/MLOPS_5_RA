...
class ChurnDataPreprocessor:
    ...
    def transform_new_data(self, X_new):
        ...
        # One-hot encoding untuk multi-class (menggunakan kolom yang sama seperti training)
        if self.one_hot_cols_multi:
            X_transformed = pd.get_dummies(X_transformed, columns=self.one_hot_cols_multi, drop_first=False)
        
        # Label encoding untuk Contract
        if 'Contract' in X_transformed.columns:
            X_transformed['Contract'] = self.label_encoder.transform(X_transformed['Contract'])
        
        # Binary encoding
        if self.one_hot_cols_binary:
            X_transformed = pd.get_dummies(X_transformed, columns=self.one_hot_cols_binary, drop_first=False)
        ...
        # Scale numeric features (harus setelah align columns)
        numeric_cols_present = [col for col in self.numeric_cols if col in X_transformed.columns]
        if numeric_cols_present:
            X_transformed[numeric_cols_present] = self.scaler.transform(X_transformed[numeric_cols_present])
